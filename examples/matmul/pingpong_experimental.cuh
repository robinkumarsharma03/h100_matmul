...
struct SMem {
    ...
    alignas(8) uint64_t full[QSIZE], empty[QSIZE], tc[2];
    int space[2][SPACE_LEN];
};

template<int BM, int BN, int BK, int NUM_THREADS, int QSIZE, int NUM_SM, int CLUSTER_M, int CLUSTER_N>
__global__  __launch_bounds__(NUM_THREADS) void  __cluster_dims__(CLUSTER_M * CLUSTER_N, 1, 1) matmulKernel10(int M, int N, int K, bf16* C, const __grid_constant__ CUtensorMap tensorMapA, const __grid_constant__ CUtensorMap tensorMapB, int* dspace) {
    ...
    int* space[2];
    space[0] = s.space[0], space[1] = s.space[1];
    
    uint32_t rank;
    asm volatile("mov.u32 %0, %clusterid.x;\n" : "=r"(rank) :);
    if (threadIdx.x < SPACE_LEN) {
        space[0][threadIdx.x] = dspace[2*rank*SPACE_LEN+threadIdx.x];
        space[1][threadIdx.x] = dspace[(2*rank+1)*SPACE_LEN+threadIdx.x];
    }
    ...
    if (threadIdx.x == 0) {
        for (int qidx = 0; qidx < QSIZE; ++qidx) {
            init_barrier(&full[qidx], 0, 1);
            init_barrier(&empty[qidx], 0, CLUSTERS);
            arrive(&empty[qidx], 2);
        }
        for (int c = 0; c < num_consumers; ++c) {
            init_barrier(&s.tc[c], 0, 1);
        }
        arrive(&s.tc[0]);
    }
    ...
    if (wg_idx == 0) {
        ...
        if (tid == 0) {
            ...
            Schedule<2, NUM_SM/CLUSTERS, BM*CLUSTER_M, BN*CLUSTER_N, 16/CLUSTER_M, 8/CLUSTER_N> schedule[2] = {
                Schedule<2, NUM_SM/CLUSTERS, BM*CLUSTER_M, BN*CLUSTER_N, 16/CLUSTER_M, 8/CLUSTER_N>(M, N, rank*2, space[0]),
                Schedule<2, NUM_SM/CLUSTERS, BM*CLUSTER_M, BN*CLUSTER_N, 16/CLUSTER_M, 8/CLUSTER_N>(M, N, rank*2+1, space[1])
            };
            bool can = true;
            int num_block_m, num_block_n;
            while (can) {
                can = false;
                // Schedule blocks for warpgroups one after another
                #pragma unroll
                for (int widx = 0; widx < num_consumers; ++widx) if (schedule[widx].next(num_block_m, num_block_n)) {
                    can = true;
                    ...
                    for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter, ++qidx) {
                        ...
                        // printf("Try filling qidx %d\n", qidx);
                        wait(&empty[qidx], p);
                        // printf("Filling qidx %d\n", qidx);
                        expect_bytes(&full[qidx], (BK*BM + BK*BN)*sizeof(bf16));
        ...
        int num_block_m, num_block_n;
        Schedule<2, NUM_SM/CLUSTERS, BM*CLUSTER_M, BN*CLUSTER_N, 16/CLUSTER_M, 8/CLUSTER_N> schedule(M, N, rank*2+(wg_idx-1), space[wg_idx]);
        while (schedule.next(num_block_m, num_block_n)) {
            ...
            // if (tid == 0) printf("Trying MMA %d\n", wg_idx);
            wait(&s.tc[wg_idx], tcp);
            // if (tid == 0) printf("Starting MMA %d\n", wg_idx);
            {
                wait(&full[qidx], p);
                warpgroup_arrive();
                #pragma unroll
                for (int m_it = 0; m_it < BM/WGMMA_M; ++m_it) {
                    bf16 *wgmma_sA = sA + qidx*BK*BM + 64*m_it*WGMMA_M;
            ...
            // if (tid == 0) printf("Finished MMA %d\n", wg_idx);
...
void runKernel10(int M, int N, int K, bf16 *A, bf16 *B, bf16 *C, int *DB) {
    ...
    if (_prev_m != M) {
        ...
        createHilbert(CEIL_DIV(M, BM*CLUSTER_M), CEIL_DIV(N, BN*CLUSTER_N), NUM_SM*2/CLUSTER_M/CLUSTER_N, space);
