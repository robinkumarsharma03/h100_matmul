...
/*
#define NVTX_SUPPRESS_V2_DEPRECATION_WARNING 1
#include <nvtx3/nvToolsExt.h>   // NVTX v3: header-only, no lib to link
...
int main() {
  ...
  for (int i = 0; i < std::size(configs); ++i) {
		...
		std::cout << "Configuration " << i+1<<" "<<m<<"x"<<n<<"x"<<k<< std::endl;
